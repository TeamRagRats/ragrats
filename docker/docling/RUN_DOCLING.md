...
2. **Migrations have run** (incl. `0008_drop_docling_document_column.sql`):
   ```bash
...
`DATABASE_URL` overrides the `.env` value so the container hits the postgres container on its internal network name (not host-localhost).
...
| `--resume` | Skip files with `status='done'` in `docling_logging` | Standard on re-run (set by the wrapper script) |
...
- **`nvrtc: invalid --gpu-architecture`** → the GPU architecture is too new for the torch wheel. Requires a rebuild with a newer torch (see the Dockerfile).
- **VLM custom code error (`trust_remote_code=True`)** → a model has been switched to one that requires custom code. Change `repo_id` in `docling_runner.py` to a standard-loader-compatible model (Granite Vision is tested).
