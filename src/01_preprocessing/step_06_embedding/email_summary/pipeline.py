...
# Per-email orchestrator for summary embedding:
#   1. Fetch emails with an OK entry in email_summaries not yet summary-chunked
#   2. For each email: embed(summary) -> upsert chunk row with strategy='summary'
...
def _process_email(
    ...
    try:
        ...
        if truncated:
            logger.warning("email %s hit max length (%d tokens); summary truncated", email_id, n_tokens)
        logger.info("email %s -> 1 chunk (%d tokens%s)", email_id, n_tokens, ", truncated" if truncated else "")
        return 1
...
def run(
    ...
    for email in emails:
        try:
            total += _process_email(conn, embed_model, tokenizer, device, run_id, email, logger)
        except Exception:
            continue
    return total
