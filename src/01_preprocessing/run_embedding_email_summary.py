...
# Entry point for summary embedding of emails (step_06_embedding/email_summary).
# Embeds the email summary from email_summaries into one vector per email and
# writes it to the chunks table as strategy='summary'. The stored chunk text
# is the summary itself.
#
# Run: python run_embedding_email_summary.py [--limit N] [--verbose]
...
from step_06_embedding.email_summary import pipeline
...
def _setup_logging(verbose: bool = False) -> logging.Logger:
    logger = logging.getLogger("email_summary_chunking")
...
def _run_pipeline(args, embed_model, tokenizer, device, logger) -> None:
    with connect() as conn:
        ...
        try:
            logger.info("=" * 60)
            logger.info("Email Summary Embedding Pipeline")
            logger.info("=" * 60)
            total = pipeline.run(conn, embed_model, tokenizer, device, run_id, logger, args.limit)
            logger.info("[email_summary] %d chunks indsat", total)
        except Exception:
            status = "failed"
            raise
        finally:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Email Summary Embedding Pipeline")
