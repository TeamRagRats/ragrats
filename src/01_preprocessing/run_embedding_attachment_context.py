...
# Entry point for context embedding of attachment documents (step_06_embedding/attachment_context).
# Loads Qwen3-Embedding-4B locally, prepends email summary to each chunk text,
# embeds each chunk independently via last-token pooling (L2-normalised), and
# writes vectors to the chunks table (source_type='attachment', strategy='context').
#
# Run: python run_embedding_attachment_context.py [--limit N] [--voyage KEY] [--verbose]
...
from step_06_embedding.attachment_context import pipeline
...
def _setup_logging(verbose: bool = False) -> logging.Logger:
    logger = logging.getLogger("attachment_context_chunking")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Attachment Context Chunking Pipeline")
    ...
    if args.sha256:
        with connect() as conn:
            run_id = start_run(conn)
            try:
                from step_06_embedding.attachment_context.pipeline import _process_attachment
