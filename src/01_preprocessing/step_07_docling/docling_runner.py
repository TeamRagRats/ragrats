...
# Wraps Docling's DocumentConverter for the run_docling.py pipeline.
# Configuration in build_docling_converter():
#   - TableFormerMode.ACCURATE set explicitly (guards against future Docling
#     default shifts).
#   - TableStructureOptions.do_cell_matching = True (aligns PDF text with
#     TableFormer's predicted grid).
#   - do_picture_description = True using IBM Granite Vision 3.3-2b as the
#     local VLM. Phi-3.5-vision was tried first but Docling's
#     PictureDescriptionVlmModel hardcodes from_pretrained without
#     trust_remote_code=True, which Phi requires; Granite Vision is one of
#     Docling's officially listed example VLMs and loads with the standard
#     transformers loader.
#   - Heron layout model selected when the installed Docling version exposes
#     ds4sd/docling-layout-heron; falls back to default layout with a logged
#     warning otherwise. Defensive imports keep the script compatible across
#     Docling minor versions.
# Persists markdown only — docling_document JSONB column was dropped on main.
...
def build_docling_converter() -> Any:
    ...
    logger.info(
        "Docling config — table_mode=ACCURATE, do_cell_matching=True, "
