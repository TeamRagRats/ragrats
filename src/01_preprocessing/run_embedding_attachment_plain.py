...
# Entry point for plain embedding of attachment documents (step_06_embedding/attachment_plain).
# Loads Qwen3-Embedding-4B locally, chunks structured_md with the shared chunker,
# embeds each chunk text directly (no email_summary prefix, no doc-level pooling),
# and writes vectors to the chunks table (source_type='attachment', strategy='plain').
# Baseline control for comparing against 'late' and 'context' strategies.
#
# Run: python -m src.01_preprocessing.run_embedding_attachment_plain [--limit N] [--voyage KEY] [--verbose]
...
from step_06_embedding.attachment_plain import pipeline
...
def _setup_logging(verbose: bool = False) -> logging.Logger:
    logger = logging.getLogger("attachment_plain_chunking")
...
def _run_pipeline(args, embed_model, tokenizer, device, logger) -> None:
    with connect() as conn:
        ...
        try:
            logger.info("=" * 60)
            logger.info("Attachment Plain Chunking Pipeline")
            logger.info("=" * 60)
            total = pipeline.run(
                conn, embed_model, tokenizer, device, run_id, logger,
                limit=args.limit, voyage=args.voyage,
            )
            logger.info("[attachment_plain] %d chunks inserted", total)
        except Exception:
            status = "failed"
            raise
        finally:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Attachment Plain Chunking Pipeline")
    ...
    parser.add_argument("--sha256", default=None, metavar="HASH",
                        help="Process a single attachment by sha256")
    ...
    if args.sha256:
        with connect() as conn:
            run_id = start_run(conn)
            try:
                from step_06_embedding.attachment_plain.pipeline import _process_attachment
