...
# Only handles phase summaries — email, thread, and fixture summaries are
# inserted directly into chunks via SQL migrations (no slicing needed).
...
from step_10_chunking.late_overlap.chunker import build_overlap_chunks
from step_10_chunking.db import get_pending_phases, upsert_chunks
...
def _run_pipeline(args: argparse.Namespace, logger: logging.Logger) -> None:
    with connect() as conn:
        ...
        try:
            logger.info("=" * 60)
            logger.info("Chunking Pipeline — phases only")
            logger.info("=" * 60)
                ...
                try:
                    ...
                    for chunk in chunks:
                        ...
                        inserted = upsert_chunks(
                            conn, "phase", source_id, voyage_key, "late_overlap", [chunk],
                        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Chunking Pipeline — phases only")
