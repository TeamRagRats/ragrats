...
# Entry point for per-email contextual summarisation (step_04 email).
# Waits for the vLLM server, then for each email generates a one-paragraph
# summary of (subject + body_cleaned) with the parent thread-so-far summary
# as context. Output is intended for an embedding model and will be paired
# with attachment summaries.
# Retries up to MAX_RETRIES times if the LLM server goes down mid-run.
# Run: python -m src.preprocessing.run_email_summaries [--voyage-key KEY] [--thread-id ID] [--email-id ID] [--limit N] [--workers N] [--verbose]
...
import step_04_summaries.email.email_summaries as step_email
...
def _setup_logging(verbose: bool = False) -> logging.Logger:
    logger = logging.getLogger("email_summaries")
...
def _run_pipeline(args: argparse.Namespace, llm: LLMClient, logger: logging.Logger) -> None:
    with connect() as conn:
        ...
        try:
            logger.info("=" * 60)
            logger.info("Email Contextual Summaries")
            logger.info("=" * 60)
            step_email.run(
                conn, run_id,
                limit=args.limit,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Email Contextual Summaries Pipeline")
    parser.add_argument("--voyage-key", type=str, default=None, metavar="KEY",
                        help="Run only for one specific voyage")
    parser.add_argument("--thread-id", type=str, default=None, metavar="ID",
                        help="Run only for one specific thread (UUID)")
    parser.add_argument("--email-id", type=str, default=None, metavar="ID",
                        help="Run only for one specific email (UUID)")
