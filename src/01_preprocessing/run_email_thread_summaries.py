...
# Entry point for per-email "thread-so-far" summarisation (step_04 email_thread).
# Waits for the vLLM server, then for each email generates a summary of the
# preceding emails in the same thread (excluding the email itself).
# Retries up to MAX_RETRIES times if the LLM server goes down mid-run.
# Run: python -m src.preprocessing.run_email_thread_summaries [--voyage-key KEY] [--thread-id ID] [--email-id ID] [--limit N] [--workers N] [--verbose]
...
import step_04_summaries.email_thread.email_thread_summaries as step_email_thread
...
def _setup_logging(verbose: bool = False) -> logging.Logger:
    logger = logging.getLogger("email_thread_summaries")
...
def _run_pipeline(args: argparse.Namespace, llm: LLMClient, logger: logging.Logger) -> None:
    with connect() as conn:
        ...
        try:
            logger.info("=" * 60)
            logger.info("Email Thread (so-far) Summaries")
            logger.info("=" * 60)
            step_email_thread.run(
                conn, run_id,
                limit=args.limit,
                llm=llm,
                logger=logger,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Email Thread-So-Far Summaries Pipeline")
