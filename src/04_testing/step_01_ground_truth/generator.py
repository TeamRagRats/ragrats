    ...
    *,
    ...
    for attempt in range(2):
        ...
        user_msg = build_user_message(
            ...
            operator_query_examples=operator_query_examples,
            retry_hint=hint,
        )
        ...
        if not _is_clean(question):
            print(f"  [warn] {category} rejected (banned token): {question[:120]}", file=sys.stderr)
            return None

        if not _mentions_vessel(question, vessel_name, voyage_key):
            if attempt == 0:
                continue
            print(f"  [warn] {category} rejected (no vessel after retry): {question[:120]}", file=sys.stderr)
            return None
        break
    else:
        return None
