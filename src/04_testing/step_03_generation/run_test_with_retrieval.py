"""
Step 2 + generation test — isolated chunk retrieval then generate and score.

Feeds the correct voyage_key from ground truth directly to retrieve_chunks
(bypassing step 1), generates an answer from the retrieved chunks, and scores
against ground_truth_answer using cosine similarity and LLM-as-judge.

Use this to measure how generation quality varies with different top_k values
in step 2, without step 1 errors contaminating the results.

Run on SPARK where both postgres, the embed server, and the LLM are reachable:
    python run_test_with_retrieval.py
    python run_test_with_retrieval.py --top-k 50
    python run_test_with_retrieval.py --top-k 100
    python run_test_with_retrieval.py --embed-url http://localhost:8003/v1
"""
...
if __name__ == "__main__" and __package__ in (None, ""):
    ...
    __package__ = "src.testing.generation.with_retrieval"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Step 2 + generation test")
    ...
    with connect() as conn:
        rows = conn.execute("""
            SELECT question_id, question, voyage_key, ground_truth_answer
            FROM ground_truth
            WHERE question_type = 'extractive'
              AND ground_truth_answer IS NOT NULL
            ORDER BY question_id
        """).fetchall()

    if not rows:
        print("No extractive ground_truth rows found.")
        return

    print(
        f"Questions: {len(rows)} | top_k: {args.top_k} | expand: ±{args.expand_window} | "
        f"embed: {args.embed_url} | llm: {args.llm_url}"
    )
    ...
    with connect() as conn:
        for i, (question_id, question, expected_key, ground_truth_answer) in enumerate(rows, 1):
            embedding = embed_client.embed([question])[0]
    ...
    print(f"\nDone. run_id={run_id}")
    print(f"top_k: {args.top_k} | expand: ±{args.expand_window}")
