...
Dimensions (per top-k round):
    ...
    hybrid     : 4 strategies x 3 rrf_k        = 12
                                       subtotal = 20
    x reformulate (2) x rerank (2)             = 80 runs / top-k round

Run three rounds (top_k = 20, 40, 60) => 240 runs. Each run covers all four
...
def _mode_variants(strategy: str, rrf_ks: list[int]) -> list[tuple[str, list[str]]]:
    """(mode label, mode args) for one strategy: vector, bm25_only, hybrid x rrf_k."""
    ...
    for rrf_k in rrf_ks:
        variants.append((
            f"mode=hybrid strategy={strategy} rrf_k={rrf_k}",
            strat + ["--hybrid", "--rrf-k", str(rrf_k)],
        ))
    return variants
        ...
        for reformulate in reformulate_opts:
            for rerank in rerank_opts:
                ...
                for strategy in STRATEGIES:
                    for mode_label, mode_args in _mode_variants(strategy, rrf_ks):
                        runs.append((
...
def main() -> None:
    ...
    runs = build_runs(top_ks, rrf_ks, reformulate_opts, rerank_opts)
