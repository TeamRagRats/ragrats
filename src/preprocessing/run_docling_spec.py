...
# Spec-variant entry point for the docling pipeline (branch docling_spec_changes).
# Mirrors run_docling.py but uses docling_runner_spec (ACCURATE table mode,
# do_cell_matching, do_picture_description with Phi-3.5-vision, Heron layout when
# available). Sequential — no parallel workers. Adds a --sha256 flag (repeatable)
# so we can re-process specific test documents without manually clearing DB rows.
...
from step_08_docling.docling_runner_spec import build_docling_converter, process_single_file
...
def _print_preflight(logger: logging.Logger, stats: dict, cuda: dict) -> None:
    logger.info("=" * 60)
    logger.info("DOCLING SPEC PRE-FLIGHT")
    logger.info("=" * 60)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Docling SPEC pipeline — sequential, ACCURATE + picture description")
    ...
    with connect() as conn:
        ...
        try:
            ...
            logger.info("Initialising Docling SPEC converter ...")
