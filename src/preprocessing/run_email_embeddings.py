...
# Entry point for email embedding (step_10 embed stage).
# Waits for the embed server, then generates embeddings for each unsummarised email.
# Retries up to MAX_RETRIES times if the embed server goes down mid-run.
# Run: python -m src.preprocessing.run_email_embeddings [--limit N] [--batch-size N] [--verbose]
...
import step_10_chunking.embed.email_embeddings as step_embed
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Email Embeddings Pipeline")
    parser.add_argument("--limit", type=int, default=None, metavar="N",
                        help="Behandl kun de første N emails (til test)")
    parser.add_argument("--batch-size", type=int, default=32,
                        help="Antal emails per embedding-kald (default: 32)")
