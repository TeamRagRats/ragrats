...
def upsert_docling(
    ...
    docling_document: Optional[dict],
    char_count: int,
    token_count: int,
) -> None:
    doc_json = json.dumps(docling_document) if docling_document is not None else None
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO docling (sha256, markdown, docling_document, char_count, token_count, processed_at) "
            ...
            "  processed_at = now()",
            (sha256, markdown, doc_json, char_count, token_count),
        )
    conn.commit()
