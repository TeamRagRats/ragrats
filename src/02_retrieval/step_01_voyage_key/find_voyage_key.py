...
def find_winning_voyage_keys(
    ...
    # Ties (same vote count) are broken on similarity — the key whose best chunk
    # is closest to the query wins (MIN(dist)). Matches the matrix harness's
...
def _find_with_candidates(
    ...
    # counts is built in similarity order (rows sorted on dist), and sorted is
